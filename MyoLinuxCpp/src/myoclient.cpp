...
namespace MYOLINUX_NAMESPACE {
    namespace myo {
         ...
         * This constructor creates the entire client stack internally. This is the preferred constructor for general use.
         * \param socket the serial socket for communication */
        Client::Client(const Serial& socket) : client(gatt::Client{bled112::Client{socket}}) { }
         ...
         */
        Client::Client(const gatt::Client &client) : client(client) { }
         ...
         * disconnect in which the connection cannot be established. To avoid this always call the disconnect method before
        ...
        Info Client::info()
        {
            return read<Info>(client, MyoInfoCharacteristic);
        }
         ...
         */
        Version Client::firmwareVersion()
        {
            return read<Version>(client, FirmwareVersionCharacteristic);
        }
         ...
         * The values are provided as received from the device.
         * To obtain the actual values they must be scaled by the appropriate scaling factor,
         * see \ref OrientationSample, \ref AccelerometerSample, \ref GyroscopeSample.
         * \param callback
         */
